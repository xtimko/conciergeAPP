...
/**
 * Image proxy: серверный fetch картинки с правильным Referer + опциональным
 * TELEGRAM_PROXY (для VPS в РФ). Решает hotlink protection и истекающие
 * signed URL: бекенд скачивает картинку один раз и кеширует в памяти,
 * фронт всегда получает её через /api/img-proxy?url=...
 *
 * SSRF защита: только http(s), не пускаем на приватные IP диапазоны.
 */
const IMG_PROXY_MAX_BYTES = 8 * 1024 * 1024;
const IMG_PROXY_CACHE_TTL_MS = 6 * 60 * 60 * 1000; // 6ч
const IMG_PROXY_CACHE_MAX_ENTRIES = 200;
const imgProxyCache = new Map(); // key=url, value={ buffer, contentType, expiresAt }
...
function evictImgProxyCacheIfNeeded() {
  const now = Date.now();
  for (const [k, v] of imgProxyCache) {
    if (v.expiresAt <= now) imgProxyCache.delete(k);
  }
  if (imgProxyCache.size > IMG_PROXY_CACHE_MAX_ENTRIES) {
    // удаляем самые старые
    const keys = [...imgProxyCache.keys()];
    const excess = imgProxyCache.size - IMG_PROXY_CACHE_MAX_ENTRIES;
    for (let i = 0; i < excess; i += 1) imgProxyCache.delete(keys[i]);
  }
}
...
app.get("/api/img-proxy", async (req, res) => {
  ...
  if (isPrivateHost(target.hostname)) return res.status(400).end();

  evictImgProxyCacheIfNeeded();
  const cached = imgProxyCache.get(raw);
  if (cached && cached.expiresAt > Date.now()) {
    ...
    res.setHeader("X-Concierge-Img-Cache", "HIT");
  ...
  try {
    ...
    // Несколько попыток: сначала с Referer = origin картинки, потом без него
    const attempts = [
      { referer: `${target.protocol}//${target.host}/`, label: "with-referer" },
      { referer: "", label: "no-referer" },
    ];

    let fetched = null;
    for (const { referer, label } of attempts) {
      try {
        const ac = new AbortController();
        const timer = setTimeout(() => ac.abort(), 30_000);
        const r = await undiciFetch(raw, {
          method: "GET",
          redirect: "follow",
          signal: ac.signal,
          headers: {
            Accept: "image/*,*/*;q=0.8",
            "User-Agent": "Mozilla/5.0 (compatible; ConciergeImgProxy/1.0)",
        ...
        clearTimeout(timer);
        if (!r.ok) {
          console.warn(`[imgProxy] ${label}: HTTP ${r.status} for ${raw.slice(0, 100)}`);
          continue;
        }
        const len = r.headers.get("content-length");
        if (len && Number(len) > IMG_PROXY_MAX_BYTES) {
          console.warn("[imgProxy] file too large");
          break;
        }
        const ab = await r.arrayBuffer();
        const buf = Buffer.from(ab);
        if (buf.length > IMG_PROXY_MAX_BYTES || buf.length === 0) continue;
        ...
        fetched = { buffer: buf, contentType: ct };
        break;
      } catch (e) {
        console.warn(`[imgProxy] ${label} error:`, e?.message || e);
      }
    }

    if (!fetched) return res.status(502).end();

    imgProxyCache.set(raw, {
      buffer: fetched.buffer,
      contentType: fetched.contentType,
      expiresAt: Date.now() + IMG_PROXY_CACHE_TTL_MS,
    });

    res.setHeader("Content-Type", fetched.contentType);
    res.setHeader("Cache-Control", "public, max-age=21600, immutable");
    res.setHeader("X-Concierge-Img-Cache", "MISS");
    res.end(fetched.buffer);
  } catch (e) {
    console.warn("[imgProxy] handler error:", e?.message || e);
    res.status(500).end();
  }
});
