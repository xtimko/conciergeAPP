...
/** Лимит файла для sendPhoto (Telegram ~10 MB). */
const MAX_TELEGRAM_PHOTO_BYTES = 10 * 1024 * 1024;

/**
 * Скачать изображение с VPS (сначала через TELEGRAM_PROXY, если задан — часто нужно для CDN из РФ;
 * затем прямое соединение).
 * @returns {Promise<{ buffer: Buffer, contentType: string } | null>}
 */
export async function fetchRemoteImageBuffer(imageUrl, maxBytes = MAX_TELEGRAM_PHOTO_BYTES) {
...
function guessFilenameFromUrl(imageUrl, contentType) {
  try {
...
/**
 * sendPhoto через multipart (байты файла). Используется, когда Telegram не может скачать photo по URL.
 ...
 * Если не вышло — качаем файл на этом сервере (в т.ч. через TELEGRAM_PROXY) и шлём multipart.
...
export async function sendTelegramPhoto(botToken, chatId, photoUrl, caption, options = {}) {
  ...
  const filename = guessFilenameFromUrl(raw, fetched.contentType);
